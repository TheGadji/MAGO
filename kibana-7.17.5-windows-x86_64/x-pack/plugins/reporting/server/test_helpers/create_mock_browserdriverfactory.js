...
const createMockBrowserDriverFactory = async (core, logger, opts = {}) => {
  const captureConfig = {
    timeouts: {
  ...
  const mockBrowserDriverFactory = _browsers.chromium.createDriverFactory(core, binaryPath, logger);

  const mockPage = {
    setViewport: () => {}
  };
  const mockBrowserDriver = new _browsers.HeadlessChromiumDriver(core, mockPage, {
    inspect: true,

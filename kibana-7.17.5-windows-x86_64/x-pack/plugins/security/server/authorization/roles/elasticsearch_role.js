...
function transformElasticsearchRoleToRole(elasticsearchRole, name, application, logger) {
  const kibanaTransformResult = transformRoleApplicationsToKibanaPrivileges(elasticsearchRole.applications, application, logger);
  return {
    name,
...
function transformRoleApplicationsToKibanaPrivileges(roleApplications, application, logger) {
  ...
  const allResources = roleKibanaApplications.filter(entry => entry.application !== _constants.RESERVED_PRIVILEGES_APPLICATION_WILDCARD).flatMap(entry => entry.resources); // if we have improperly formatted resource entries, we can't transform these

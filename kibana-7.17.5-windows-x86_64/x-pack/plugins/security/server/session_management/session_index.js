...
function getSessionIndexTemplate(indexName) {
...
class SessionIndex {
  ...
  async invalidate(filter) {
    ...
    try {
      const {
        body: response
      } = await this.options.elasticsearchClient.deleteByQuery({
        index: this.indexName,
        refresh: true,
        body: {
          query: deleteQuery
  ...
  async cleanUp() {
    ...
    try {
      ...
      if (response.deleted > 0) {
        this.options.logger.debug(`Cleaned up ${response.deleted} invalid or expired session values.`);
      }
    } catch (err) {

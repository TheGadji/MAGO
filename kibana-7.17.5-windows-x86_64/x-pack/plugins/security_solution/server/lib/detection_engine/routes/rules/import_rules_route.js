...
const importRulesRoute = (router, config, ml, isRuleRegistryEnabled) => {
  router.post({
    ...
    try {
      ...
      let parsedRules;
      let actionErrors = [];
      const actualRules = parsedObjects.filter(rule => !(rule instanceof Error));
      ...
      while (chunkParseObjects.length) {
        ...
        importRuleResponse = [...actionErrors, ...duplicateIdErrors, ...importRuleResponse, ...newImportRuleResponse];
      }

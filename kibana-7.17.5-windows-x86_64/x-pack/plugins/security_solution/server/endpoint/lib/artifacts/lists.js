...
var _validations = require("../../../../common/endpoint/service/trusted_apps/validations");
/*
...
function getMatcherFunction({
  ...
  return matchAny ? field.endsWith('.caseless') ? 'exact_caseless_any' : 'exact_cased_any' : field.endsWith('.caseless') ? os === 'linux' ? 'exact_cased' : 'exact_caseless' : 'exact_cased';
}
...
function getMatcherWildcardFunction({
  ...
  return field.endsWith('.caseless') ? os === 'linux' ? 'wildcard_cased' : 'wildcard_caseless' : 'wildcard_cased';
}
...
function translateEntry(schemaVersion, entry, os) {
  switch (entry.type) {
    case 'nested':
      {
    ...
    case 'wildcard':
      {
        ...
        const buildEntries = () => {
          if (translatedEntryWildcardMatcher) {
            ...
            const hasExecutableName = (0, _validations.hasSimpleExecutableName)({
              os: os,
              type: entry.type,
              value: entry.value
            });

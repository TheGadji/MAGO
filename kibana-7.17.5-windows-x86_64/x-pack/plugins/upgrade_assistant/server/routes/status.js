...
function registerUpgradeStatusRoute({
  ...
  router.get({
    ...
    try {
      ...
      } = await (0, _es_deprecations_status.getESUpgradeStatus)(esClient); // Fetch system indices migration status
      ...
      const notMigratedSystemIndices = features.filter(feature => feature.migration_status !== 'NO_MIGRATION_NEEDED').length; // Fetch Kibana upgrade status

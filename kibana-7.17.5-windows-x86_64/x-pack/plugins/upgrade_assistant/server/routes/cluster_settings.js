...
function registerClusterSettingsRoute({
  ...
  router.post({
    ...
    try {
      const {
        settings
      } = request.body; // We need to fetch the current cluster settings in order to determine
      // if the settings to delete were set as transient or persistent settings

      const {
        body: currentClusterSettings
      ...
      } = await client.asCurrentUser.cluster.putSettings({

...
const reindexServiceFactory = (esClient, actions, log, licensing) => {
  ...
  const createNewIndex = async reindexOp => {
    var _createIndex, _createIndex$body;
    ...
    try {
      ...
      var _err$body, _err$body$error; // If for any reason the new index name generated by the `generateNewIndexName` already
    ...
    if (createIndex && !((_createIndex = createIndex) !== null && _createIndex !== void 0 && (_createIndex$body = _createIndex.body) !== null && _createIndex$body !== void 0 && _createIndex$body.acknowledged)) {
  ...
  const updateReindexStatus = async reindexOp => {
    ...
    if (!taskResponse.completed) {
      ...
      var _taskResponse$respons, _taskResponse$respons2; // Check that no failures occurred
  ...
  return {
    ...
    async detectReindexWarnings(indexName) {
      ...
      if (!flatSettings) {
        ...
        return [// By default all reindexing operations will replace an index with an alias (with the same name)

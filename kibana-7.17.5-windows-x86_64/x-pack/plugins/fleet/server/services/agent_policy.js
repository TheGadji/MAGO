...
class AgentPolicyService {
  ...
  async ensurePreconfiguredAgentPolicy(soClient, esClient, config) {
    ...
    let searchParams;
    const isDefaultPolicy = preconfiguredAgentPolicy.is_default || preconfiguredAgentPolicy.is_default_fleet_server;
  ...
  async ensureAgentPolicy(soClient, esClient, newAgentPolicy, searchParams, id) {
    ...
    if (agentPolicies.total === 0) {
      return {
        created: true,
        policy: await this.create(soClient, esClient, newAgentPolicy, {
          id: id ? String(id) : (0, _v2.default)(newAgentPolicy.name, _common.UUID_V5_NAMESPACE)
        })
      };
    }
  ...
  async delete(soClient, esClient, id, options) {
    ...
    if (agentPolicy.is_default && !(options !== null && options !== void 0 && options.force)) {
      throw new Error('The default agent policy cannot be deleted');
    }

    if (agentPolicy.is_default_fleet_server && !(options !== null && options !== void 0 && options.force)) {
  ...
  async createFleetServerPolicy(soClient, agentPolicyId) {

...
class PackagePolicyService {
  async create(soClient, esClient, packagePolicy, options) {
    var _packagePolicy$packag, _options$user$usernam, _options$user, _options$user$usernam2, _options$user2, _options$bumpRevision;

    if (!(options !== null && options !== void 0 && options.skipUniqueNameVerification)) {
      const existingPoliciesWithName = await this.list(soClient, {
        perPage: 1,
        kuery: `${_constants.PACKAGE_POLICY_SAVED_OBJECT_TYPE}.name: "${packagePolicy.name}"`
      }); // Check that the name does not exist already

      if (existingPoliciesWithName.items.length > 0) {
        throw new _errors.IngestManagerError('There is already an integration policy with the same name');
      }
    }
    ...
    if ((_packagePolicy$packag = packagePolicy.package) !== null && _packagePolicy$packag !== void 0 && _packagePolicy$packag.name) {
      ...
      inputs = await this._compilePackagePolicyInputs(registryPkgInfo, pkgInfo, packagePolicy.vars || {}, inputs);
  ...
  async update(soClient, esClient, id, packagePolicy, options) {
    ...
    if (packagePolicy.package) {
      ...
      _.appContextService.getLogger().info(`Package policy successfully upgraded ${JSON.stringify(upgradeMeta)}`, upgradeMeta);
    }
  ...
  async getUpgradeDryRunDiff(soClient, id) {
    try {
      ...
      if (packagePolicy.package.version !== packageInfo.version) {
        ...
        _.appContextService.getLogger().info(`Package policy upgrade dry run ${hasErrors ? 'resulted in errors' : 'ran successfully'} ${JSON.stringify(upgradeMeta)}`, upgradeMeta);
      }
  ...
  async _compilePackagePolicyInputs(registryPkgInfo, pkgInfo, vars, inputs) {
...
async function _compilePackagePolicyInput(registryPkgInfo, pkgInfo, vars, input) {
...
async function _compilePackageStreams(registryPkgInfo, pkgInfo, vars, input) {
  const streamsPromises = input.streams.map(stream => _compilePackageStream(registryPkgInfo, pkgInfo, vars, input, stream));
...
async function _compilePackageStream(registryPkgInfo, pkgInfo, vars, input, streamIn) {
  let stream = streamIn;
  ...
  for (const preconfiguredInput of preconfiguredInputs) {
    ...
    if (preconfiguredInput.streams) {
      for (const stream of preconfiguredInput.streams) {
        var _originalInput2, _originalStream2;
        ...
        if (((_originalStream2 = originalStream) === null || _originalStream2 === void 0 ? void 0 : _originalStream2.enabled) === undefined) {
          originalStream.enabled = stream.enabled;
        }
...
async function incrementPackageName(soClient, packageName) {
  ...
  const pkgPoliciesWithMatchingNames = packagePolicyData !== null && packagePolicyData !== void 0 && packagePolicyData.items ? packagePolicyData.items.filter(ds => Boolean(ds.name.match(pkgPoliciesNamePattern))).map(ds => parseInt(ds.name.match(pkgPoliciesNamePattern)[1], 10)).sort((a, b) => a - b) : [];
  return `${packageName}-${pkgPoliciesWithMatchingNames.length ? pkgPoliciesWithMatchingNames[pkgPoliciesWithMatchingNames.length - 1] + 1 : 1}`;
}
...
class TaskPollingLifecycle {
  ...
  constructor({
    ...
    (0, _defineProperty2.default)(this, "pollForWork", async (...tasksToClaim) => {
      ...
      () => {
        return claimAvailableTasks(tasksToClaim.splice(0, this.pool.availableWorkers), this.taskClaiming, this.logger).pipe((0, _operators.tap)((0, _result_type.mapOk)(({
          timing
        }) => {
          if (timing) {
      ...
      async tasks => {
        const result = await this.pool.run(tasks); // Emit the load after fetching tasks, giving us a good metric for evaluating how
  ...
  return new _rxjs.Observable(observer => {
    taskClaiming.claimAvailableTasksIfCapacityIsAvailable({
      ...
      if ((0, _identify_es_error.isEsCannotExecuteScriptError)(ex)) {
        ...
        const esError = (0, _identify_es_error.identifyEsError)(ex); // as we could't identify the reason - we'll error out the wrapping Observable too

...
const useField = (form, path, config = {}, valueChangeListener, errorChangeListener, {
  ...
  const debounceTimeout = (0, _react.useRef)(null); // Keep a ref of the last state (value and errors) notified to the consumer so he does
  ...
  const onValueChange = (0, _react.useCallback)(async () => {
    ...
    setPristine(false);
    setIsChangingValue(true); // Update the form data observable
  ...
  }, [path, value, valueChangeDebounceTime, fieldsToValidateOnChange, __updateFormDataAt, __validateFields]); // Cancel any inflight validation (e.g an HTTP Request)
  ...
  const runValidations = (0, _react.useCallback)(({
    ...
    clearFieldErrors([validationTypeToValidate !== null && validationTypeToValidate !== void 0 ? validationTypeToValidate : _constants.VALIDATION_TYPES.FIELD, _constants.VALIDATION_TYPES.ASYNC]);
    ...
    let dataProvider = () => Promise.resolve(null);

    if (customValidationData$) {
      dataProvider = () => customValidationData$.pipe((0, _operators.first)()).toPromise();
    }
    ...
    return runSync();
  }, [cancelInflightValidation, validations, getFormData, getFields, path, customValidationData, customValidationData$]); // ----------------------------------
  // -- Internal API
  // ----------------------------------
  ...
  }, [deserializeValue, defaultValue, setValue, setStateErrors]); // Don't take into account non blocker validation. Some are just warning (like trying to add a wrong ComboBox item)
  ...
  }, [path, __removeField]); // Notify listener whenever the value changes

...
class DefaultSearchCapabilities {
  ...
  get whiteListedMetrics() {
    if (this.panel && this.panel.type !== _enums.PANEL_TYPES.TIMESERIES && this.panel.time_range_mode === _enums.TIME_RANGE_DATA_MODES.ENTIRE_TIME_RANGE) {
      const aggs = (0, _agg_utils.getAggsByType)(agg => agg.id);
      const allAvailableAggs = [...aggs[_agg_utils.AGG_TYPE.METRIC], ...aggs[_agg_utils.AGG_TYPE.SIBLING_PIPELINE], _enums.TSVB_METRIC_TYPES.MATH, _enums.TSVB_METRIC_TYPES.CALCULATION, _enums.BUCKET_TYPES.TERMS, // SERIES_AGG should be blocked for table
      ...(this.panel.type === _enums.PANEL_TYPES.TABLE ? [] : [_enums.TSVB_METRIC_TYPES.SERIES_AGG])].reduce((availableAggs, aggType) => ({ ...availableAggs,
        [aggType]: {
          '*': true
        }
      }), {});

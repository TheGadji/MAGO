...
exports.migrateExplicitlyHiddenTitles = exports.createDashboardSavedObjectTypeMigrations = void 0;
...
const migrateExplicitlyHiddenTitles = doc => {
  const {
    attributes
  } = doc; // Skip if panelsJSON is missing
  ...
  try {
    ...
    panels.forEach(panel => {
      ...
      const originalPanelState = (0, _embeddable_saved_object_converters.convertSavedDashboardPanelToPanelState)(panel);
      newPanels.push((0, _embeddable_saved_object_converters.convertPanelStateToSavedDashboardPanel)({ ...originalPanelState,
        explicitInput: { ...originalPanelState.explicitInput,
...
const createDashboardSavedObjectTypeMigrations = deps => {
  ...
  const dashboardMigrations = {
    ...
    '7.17.3': (0, _lodash.flow)(migrateExplicitlyHiddenTitles)
  };
